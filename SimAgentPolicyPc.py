...
sheet_name = "sub"
# 创建情景序列
pc_wheat_list = np.arange(2.34, 2.441, 0.01)
pc_maize_list = np.arange(1.61, 1.641, 0.003)
eta_list = create_eta_list(eta0)

all_TW_total1 = {}
all_TW_total2 = {}
for pc_wheat in pc_wheat_list:
    print(f"\n正在模拟小麦价格 pc_wheat = {pc_wheat:.5f}")
    crop_types, farmers, betas, s1_0, TW_0 = initialize_simulation(eta0, pw0, crop_params_file, sheet_name)
    # 找出所有self.type == 'Wheat'的农民，修改其self.pc
    for farmer in farmers['Wheat']:
        farmer.pc = pc_wheat
    w_star_range, s1_total, TW_total, yield_total = run_simulation(crop_types, farmers, eta_list, pw0)
    all_TW_total1[pc_wheat] = TW_total

for pc_maize in pc_maize_list:
    print(f"\n正在模拟玉米价格 pc_maize = {pc_maize:.5f}")
    crop_types, farmers, betas, s1_0, TW_0 = initialize_simulation(eta0, pw0, crop_params_file, sheet_name)
    # 找出所有self.type == 'Maize'的农民，修改其self.pc
    for farmer in farmers['Maize']:
        farmer.pc = pc_maize
    w_star_range, s1_total, TW_total, yield_total = run_simulation(crop_types, farmers, eta_list, pw0)
    all_TW_total2[pc_maize] = TW_total

#%% 绘制图表
import plot_utils
importlib.reload(plot_utils)
from plot_utils import plot_pc_water_use_heatmap

# 绘制热力图
print("\n绘制总用水量热力图")

total_water_matrix1,  total_water_matrix2 = plot_pc_water_use_heatmap(eta_list, pc_wheat_list, pc_maize_list,all_TW_total1, all_TW_total2, crop_types)
# %%
